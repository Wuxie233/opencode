...
- Keep provider-native compaction windows opaque and Session-owned. Persist them by Session/provider/model/route/variant with a strictly increasing EventV2 aggregate sequence; legacy compaction markers must store that same sequence so V1 and current runners share one CAS ordering domain. Replay current system context before the canonical items, then any pre-marker tail explicitly retained by `tail_start_id`, followed by transcript entries newer than the marker. Full projected history remains authoritative for UI, revert, model or agent switches, and local fallback; those lifecycle changes clear canonical state. `compaction.mode: "local"` is the provider-compaction kill switch.
