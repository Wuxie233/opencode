...
- Browser draft Blob garbage collection must use an IndexedDB object cursor when deleting entries; key cursors cannot delete records. Keep `blob:` in the Web UI `img-src` CSP for local previews, and ship `site.webmanifest` as valid JSON rather than relying on materialized symlinks.
...
- Share an MCP client across directory instances only when config provenance proves the effective entry is global, the transport is remote, and OAuth is explicitly disabled. Project or runtime overrides, local transports, and OAuth-capable entries remain directory-scoped; shared requests carry directory context per invocation and shared clients do not advertise LSP-style roots.
- Reuse LSP clients by normalized workspace root, server identity, and effective launch configuration. Never share one language-server session across unrelated roots; retain active operation borrows and reclaim idle clients after 10 minutes.
