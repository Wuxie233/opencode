...
import { Context, Effect, Layer, RcRef, Scope } from "effect"
import { Fff } from "#fff"
...
export const fffLayer = Layer.effect(
  ...
  Effect.gen(function* () {
    const location = yield* Location.Service
    const backend = yield* RcRef.make({
      acquire: Effect.acquireRelease(
        Effect.gen(function* () {
          const picker = yield* Effect.try({
            ...
            Effect.tap(() => Effect.logInfo("initialized fff search")),
            Effect.map((picker) => ({ type: "fff" as const, picker, service: fffService(picker) })),
            Effect.catch(() =>
              Effect.logWarning("failed to initialize fff; using ripgrep").pipe(
                Effect.andThen(makeRipgrep),
                Effect.map((service) => ({ type: "ripgrep" as const, service })),
              ),
            ),
          )
          return picker
        ...
        (current) => {
          if (current.type !== "fff") return Effect.void
          return Effect.sync(() => current.picker.destroy()).pipe(
            Effect.ignore,
            Effect.andThen(Effect.logInfo("disposed fff search")),
          )
        },
      ),
      idleTimeToLive: "10 minutes",
    })
    const use = <A>(run: (search: Interface) => Effect.Effect<A>) =>
      Effect.scoped(RcRef.get(backend).pipe(Effect.flatMap((current) => run(current.service))))
    return Service.of({
      glob: (input) => use((search) => search.glob(input)),
      grep: (input) => use((search) => search.grep(input)),
      find: (input) => use((search) => search.find(input)),
    })
  }),
)

function fffService(picker: Fff.Picker) {
  return Service.of({
    glob: (input) =>
      Effect.sync(() => {
