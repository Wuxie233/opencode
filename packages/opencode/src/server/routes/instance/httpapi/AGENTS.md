...
Use `request-performance.ts` for request timing. Keep event fields low-cardinality and privacy-safe: method, normalized `RouteContext.route.path`, status, integer durations, and fixed phase names only. Time to response-ready is distinct from streaming body lifetime. Never inspect or log the request URL, params, query, headers, directory, payload, response body, or session data.
