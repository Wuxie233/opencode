...
// Raw catch-all for the reserved `/api/plugin/<pluginID>/*` namespace. The routed
// instance directory comes from `InstanceState.directory`, which the router-level
// workspace-routing + instance-context middleware provide per request, so the same
// pluginID resolves to the handler registered for the routed instance.
