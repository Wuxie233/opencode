...
- Encode `x-opencode-directory` with `encodeURIComponent` before sending it to remote MCP transports. Receivers decode it exactly once; raw Unicode is not a valid HTTP header value, and literal percent sequences must round-trip unchanged.
