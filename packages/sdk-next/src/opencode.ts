...
export const create = Effect.fn("OpenCode.create")(function* () {
  ...
  const context = yield* Layer.buildWithMemoMap(
    AppNodeBuilder.build(LayerNode.group([ApplicationTools.node, Attachment.node, PermissionSaved.node])),
    memoMap,
    scope,
  )
  const tools = Context.get(context, ApplicationTools.Service)
  const attachments = Context.get(context, Attachment.Service)
  const permissions = Context.get(context, PermissionSaved.Service)
  const requestContext = Context.make(Attachment.Service, attachments)
  ...
  const fetch = Object.assign(
    (input: RequestInfo | URL, init?: RequestInit) => web.handler(new Request(input, init), requestContext),
