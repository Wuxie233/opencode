...
test("serves the server-backed web state contract", async () => {
  ...
  const request = async (method: string, record?: unknown) => {
    let response = ""
    await handler!({
      request: () => ({
        url: () => "http://127.0.0.1:4096/api/plugin/opencode.web-state/state/server.projects",
        method: () => method,
        headers: () => ({}),
        postDataJSON: () => record,
      }),
  ...
  expect(await request("GET")).toEqual({ value: null, version: "v1", updated_at: 0 })
