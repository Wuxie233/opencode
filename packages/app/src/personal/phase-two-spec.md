...
One operator moves repeatedly between several open projects and sessions, needs to see active or blocked work quickly, opens recent history without returning Home, and uses the same interface from desktop and mobile Web.
...
- Combine native persisted tabs with recent non-archived root sessions from the existing V2 Home session-index path.
...
- Add project collapse, session search, active/open state, and bounded recent-session ordering.
...
- Session history reuses `loadHomeSessionIndex` and the per-server TanStack query caches. Do not add a second backend API or persistence store.
...
- Desktop at 1080px and above shows the persistent rail. The native titlebar remains the current open-tab surface; the rail is project/history/status navigation rather than a visual duplicate.
...
- Recent sessions, persisted tabs, new drafts, open/select, close-tab, and archive actions use native contexts and keep cross-server identities correct.
