...
export async function bootstrapDirectory(input: {
  ...
  const slow = (mode === "full" ? [...recovery, ...full] : recovery).filter(Boolean) as (() => Promise<any>)[]
  ...
  if (loading && mode === "full" && slowErrs.length === 0) input.setStore("status", "complete")
}
