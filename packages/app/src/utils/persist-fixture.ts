...
// local-to-server migration, settings/tabs sync, fallback) can share setup
// instead of re-declaring an in-memory localStorage double and a server-state
// mock per file.
//
// Nothing here implements the real persistence adapter, transport, or plugin
// storage API. These are test doubles only.

// In-memory `Storage` double mirroring the localStorage behavior used by persist.ts.
// Each instance is isolated; call `reset()` in beforeEach to avoid cross-test
// leakage. Keys prefixed with "opencode.quota" throw a QuotaExceededError on set;
// keys prefixed with "opencode.throw" throw on every op, matching the
// failure-injection contract the existing persist tests rely on.
...
// Returns the storage so callers can assert on it.
...
export type ServerRecord = {
  value: unknown
  // Contract version string (e.g. "v1"). The LWW conflict policy below
  // intentionally does NOT use this; it uses numeric `updated_at` only.
  version: string
  updated_at: number
}

// In-memory double for the server-side UI-state plugin store. Lets app
// persistence tests exercise server-first hydrate, last-write-wins, and migration
// without real HTTP or a running backend.
//
// Isolation: each instance owns its own record map. LWW: `put` only overwrites
// when the incoming `updated_at` is strictly newer than the stored record, so
// stale writes cannot clobber newer state.
export class MockServerState {
  private records = new Map<string, ServerRecord>()
  readonly calls = { get: 0, put: 0, list: 0, remove: 0 }
  // When set, every store call throws to simulate plugin/server unavailable.
  unavailable = false
...
// Sub-namespace any record carrying an x-opencode-directory header by directory,
// so workspace/session groups are isolated per directory while global groups
// (no header) stay shared.
...
// A `fetch` double that serves the `opencode.web-state` GET/PUT contract against a
// MockServerState, mirroring the real plugin route's shapes.
export function mockServerStateFetch(server: MockServerState): typeof fetch {
  const fetcher = (async (input, init) => {
    ...
    const name = stateName(decodeURIComponent(rawName), input, init)
