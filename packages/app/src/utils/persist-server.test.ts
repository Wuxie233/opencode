...
import { installLocalStorage, MemoryStorage, mockServerStateFetch, MockServerState } from "./persist-fixture"
import { ServerScope, type ServerScope as ServerScopeValue } from "./server-scope"
...
let fetcher: typeof fetch = mockServerStateFetch(server)
...
beforeAll(async () => {
  mock.module("@/context/platform", () => ({
    usePlatform: () => ({ platform: "web", fetch: fetcher }),
  }))
...
beforeEach(() => {
  ...
  fetcher = mockServerStateFetch(server)
  unregisterResolver = registerWebStateServerResolver((scope) =>
    scope === SCOPE ? { url: "http://canonical-local.test", fetch: fetcher } : undefined,
  )
  clock = 1_000
  Date.now = () => clock
})
...
function mount(input?: {
  ...
  if (input?.fetch) fetcher = input.fetch
...
describe("persist server hydrate", () => {
  ...
  test("local scope uses the registered canonical server resolver", async () => {
    ...
    expect(server.calls.get).toBeGreaterThan(0)
    expect(storage.getItem("local-resolver")).toBe('{"count":6}')
    result.dispose()
  })

  test("remote scope without a resolver stays local-only", async () => {
    seedLocal("remote-unresolved", { count: 9 }, 9)

    const result = mount({ key: "remote-unresolved", scope: REMOTE_SCOPE })
    await waitReady(result)
...
describe("persist fallback", () => {
  ...
  test("fallback keeps local state when server payload is malformed", async () => {
    ...
    const result = mount({
      ...
      fetch: (() =>
        Promise.resolve(
          new Response(JSON.stringify({ value: { count: 9 }, version: 1, updated_at: 9 }), {
            status: 200,
  ...
  test("fallback ignores malformed server payload when local fallback is empty", async () => {
    const result = mount({
      ...
      fetch: (() =>
        Promise.resolve(
          new Response(JSON.stringify({ value: { count: 9 }, version: "v1", updated_at: "bad" }), {
            status: 200,
            headers: { "content-type": "application/json" },
          }),
        )) as unknown as typeof fetch,
...
describe("persist directory routing", () => {
